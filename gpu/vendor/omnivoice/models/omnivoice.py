...
The upstream class retains its ``forward()`` implementation for model
compatibility, but this vendored runtime subset intentionally excludes
OmniVoice's training entrypoints.
"""
...
class OmniVoice(PreTrainedModel):
    ...
    def _post_process_audio(
        ...
        if postprocess_output:
            ...
            # VivaVoce local change:短/低能量语音经 remove_silence 后可能整段被判静音
            #   → 返回 0 长度数组 → 下面 np.abs(...).max() 报「zero-size array to reduction operation
            #   maximum which has no identity」→ TTS 引擎崩 → 媒体面通道被拆 → AI 几秒退会、全程无声。
            #   若去静音把音频清空了,保留**去静音前**的原始波形(宁可带点静音,也不要空音频崩引擎)。
            if np.asarray(desilenced).size > 0:
                generated_audio = desilenced

        # 兜底:仍为空(理论上不该到,防御)→ 跳过归一化,交回上层(funasr_backend 会判空并报可读错)。
        if np.asarray(generated_audio).size == 0:
